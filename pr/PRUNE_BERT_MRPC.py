...
address = './output/PRUNE_BERT_mrpc_no/'
if not os.path.exists(address):
    os.makedirs(address)
f_name = 'PR_BERT_mrpc_no'+'lr_'+ 'adam'+\
...
model_1 = './model/saved_bert_model_weights_mrpc.pth'
...
dataset = load_dataset('./data/mrpc')
print(dataset)
...
for prune_rate in tqdm(prune_rates):
    ...
    avg_loss, accuracy, f1 = evaluate(model, val_dataloader, device)

    print(f"Prune Rate: {prune_rate:.4f},Test Loss: {avg_loss:.4f},Validation ACC: {accuracy:.4f},F1: {f1:.4f}")
    file4.writelines(str(avg_loss)+' '+str(accuracy)+' '+str(f1)+'  '+str(round(prune_rate,4))+'\n')
