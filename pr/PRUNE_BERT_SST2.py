...
address = './output/PRUNE_BERT_sst2_no/'
if not os.path.exists(address):
    os.makedirs(address)

f_name = 'PR_BERT_sst2' + 'lr_' + 'adam' + \
...
model_1 = './model/saved_bert_model_weights_sst2.pth'
...
dataset = load_dataset('./data/sst2')
print(dataset)

tokenizer = BertTokenizer.from_pretrained('./model/bert-base-uncased/')

def preprocess_function(examples):
    return tokenizer(examples['sentence'], truncation=True, padding='max_length', max_length=128, return_tensors='pt')
...
for name, param in model.named_parameters():   
    ...
    if name == 'bert.encoder.layer.11.intermediate.dense.weight':
        cos_similarity_ffn_pr(name)
    if name == 'bert.encoder.layer.11.output.dense.weight':
        cos_similarity_ffn_pr(name)
file3.writelines('\n') 
